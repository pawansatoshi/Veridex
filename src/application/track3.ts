...
function tryParseJson(value: unknown): unknown {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Unstructured responses remain inconclusive.
    }
  }
  return value;
}
...
function extractReview(raw: unknown): ParsedTelegraphReview {
  ...
  const reasons = Array.isArray(rawReasons)
    ...
    : reason ? [reason] : [];
  return { assessment: normalizedAssessment, riskLevel, confidence: clampConfidence(candidate.confidence), reasons };
}
...
function decide(analysis: NormalizedAnalysis, review: ParsedTelegraphReview, telegraphAvailable: boolean): Track3Decision {
  if (!telegraphAvailable) return { status: "DETERMINISTIC_ONLY", label: "Deterministic evidence only", rationale: "Telegraph intelligence was not available, so no secondary conclusion was substituted for the primary evidence model." };
  if (review.assessment === "contradicts") return { status: "CONFLICTED", label: "Evidence conflict requires review", rationale: "The live Telegraph review explicitly contradicted the deterministic posture. Veridex preserves the disagreement instead of forcing a single security conclusion." };
  if (review.assessment === "supports" && analysis.conclusive) return { status: "CORROBORATED", label: "Deterministic evidence corroborated", rationale: "The deterministic evidence is conclusive under the Veridex model and the live Telegraph review independently supports that posture." };
  if (!analysis.conclusive || review.assessment === "inconclusive" || review.assessment === "invalid") return { status: "INCONCLUSIVE", label: "Evidence remains incomplete", rationale: "The available evidence does not justify a fully corroborated conclusion." };
  return { status: "DETERMINISTIC_ONLY", label: "Deterministic evidence leads", rationale: "The deterministic evidence is usable, but the Telegraph review did not produce a compatible structured assessment." };
}

function telegraphStatus(result: TelegraphAskResult | undefined): "settled" | "required" | "not_required" | "unavailable" | "failed" {
...
export async function runTrack3Analysis(body: unknown): Promise<Track3Result> {
  ...
  const signals = analysis.capabilities.map((item) => ({ capability: item.capability, result: item.result, confidence: item.confidence, conclusive: item.conclusive, evidenceKind: item.evidence.kind }));
  ...
  try {
    telegraphResult = await askTelegraph(reviewQuery.query, reviewQuery.context, loadTelegraphClientOptions());
  } catch (error) {
  ...
  const evidence: Track3Result["evidence"] = [
    { source: "veridex", type: "deterministic", status: analysis.capabilities.length > 0 ? "available" : "inconclusive", summary: "Primary contract observations from the Veridex evidence model.", details: { conclusive: analysis.conclusive, confidence: analysis.confidence, capabilityCount: analysis.capabilities.length, signals } },
    { source: "telegraph", type: "independent_review", status: telegraphAvailable && review.assessment !== "invalid" ? "available" : "inconclusive", summary: telegraphAvailable ? "Live Telegraph Miner response used as a secondary review signal." : "No usable live Telegraph review was available; this is not treated as a negative finding.", details: telegraphErrorMessage ? { error: telegraphErrorMessage } : { assessment: review.assessment, riskLevel: review.riskLevel, confidence: review.confidence } },
  ];
  const telegraph: Track3Result["telegraph"] = {
    status: telegraphStatus(telegraphResult),
    review,
    rawResult: telegraphResponse?.result ?? null,
  };
  const intent = text(telegraphResponse?.intent);
  const miner = text(telegraphResponse?.miner_used);
  const minerName = text(telegraphResponse?.miner_name);
  if (intent) telegraph.intent = intent;
  if (miner) telegraph.miner = miner;
  if (minerName) telegraph.minerName = minerName;
  if (typeof telegraphResponse?.cost_usd === "number") telegraph.costUsd = telegraphResponse.cost_usd;
  if (typeof telegraphResponse?.duration_ms === "number") telegraph.durationMs = telegraphResponse.duration_ms;
  if (telegraphResult?.metadata.paymentNetwork) telegraph.paymentNetwork = telegraphResult.metadata.paymentNetwork;
  if (telegraphResult?.metadata.paymentAmountAtomic) telegraph.paymentAmountAtomic = telegraphResult.metadata.paymentAmountAtomic;
  if (telegraphResult?.metadata.paymentProof) telegraph.paymentProof = telegraphResult.metadata.paymentProof;

  return { schema: "veridex.track3.application.v1", requestId, analysis, capabilityIntelligence, capabilityPassport, telegraph, decision, evidence };
}
