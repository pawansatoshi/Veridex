...
export function diffCapabilities(before: NormalizedAnalysis, after: NormalizedAnalysis): CapabilityDiff {
  ...
  for (const capability of names) {
    ...
    const change = beforeResult === "negative" && afterResult === "positive" ? "added" : beforeResult === "positive" && afterResult === "negative" ? "removed" : "changed";
  ...
  const confidence = changes.length === 0 ? Math.min(before.confidence, after.confidence) : changes.reduce((sum, change) => sum + Math.min(before.capabilities.find((item) => item.capability === change.capability)?.confidence ?? 0, after.capabilities.find((item) => item.capability === change.capability)?.confidence ?? 0), 0) / changes.length;
  return { changed: changes.length > 0, changes, confidence, conclusive: before.conclusive && after.conclusive && changes.every((change) => !["inconclusive", "unavailable"].includes(change.before) && !["inconclusive", "unavailable"].includes(change.after)) };
}
