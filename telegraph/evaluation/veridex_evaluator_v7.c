...
static int group(const uint8_t*p,int n){
 ...
 {"fraud","fraudulent","scam","phishing","phish","malicious","malware","fake","counterfeit","deceptive","deception","dangerous","harmful","attack","attacker","compromised","stolen","ponzi"},
 ...
 static const uint8_t C[]={18,11,6,6,9,9,5,4,22,20};
 for(int g=0;g<10;g++) for(int i=0;i<C[g];i++) if(eq(p,n,G[g][i])) return g+1; return 0;
}
static int opposite(int g1,int g2){ return (g1==1&&g2==2)||(g1==2&&g2==1)||(g1==3&&g2==4)||(g1==4&&g2==3)||(g1==5&&g2==6)||(g1==6&&g2==5)||(g1==7&&g2==8)||(g1==8&&g2==7)||(g1==9&&g2==10)||(g1==10&&g2==9); }
...
static int parse_numbers(const uint8_t*s,int n,double*v,int*pct,int cap){
  int p=0,c=0; while(p<n&&c<cap){ while(p<n&&!digit(s[p])&&s[p]!='.')p++; if(p>=n)break; double x=0.0; int any=0,dot=0; double place=.1;
    ...
    if(!any){p++;continue;} while(p<n&&space(s[p]))p++;
    if(p<n&&(s[p]=='k'||s[p]=='K')){x*=1000;p++;} else if(p<n&&(s[p]=='m'||s[p]=='M')){x*=1000000;p++;} else if(p<n&&(s[p]=='b'||s[p]=='B')){x*=1000000000;p++;} else {int e=p;while(e<n&&word(s[e]))e++;if(e-p==7&&eq(s+p,7,"million")){x*=1000000;p=e;}else if(e-p==8&&eq(s+p,8,"thousand")){x*=1000;p=e;}else if(e-p==6&&eq(s+p,6,"trillion")){x*=1000000000000.0;p=e;}else if(e-p==7&&eq(s+p,7,"billion")){x*=1000000000;p=e;}}
    pct[c]=(p<n&&s[p]=='%'); if(pct[c])p++; v[c++]=x;
  } return c;
}
static int numeric_quality(const uint8_t*gt,int gn,const uint8_t*ans,int an){
  int na=parse_numbers(gt,gn,num_a,pct_a,MAX_NUMBERS),nb=parse_numbers(ans,an,num_b,pct_b,MAX_NUMBERS);if(na==0&&nb==0)return 0;if(na==0||nb==0||na!=nb)return -1;
  for(int i=0;i<na;i++){double s=num_a[i]>num_b[i]?num_a[i]:num_b[i];if(s<1)s=1;double d=num_a[i]>num_b[i]?num_a[i]-num_b[i]:num_b[i]-num_a[i];if(pct_a[i]!=pct_b[i]||d>s*.001+1e-6)return -1;}return 1;
}
static int likely_entity(const uint8_t*s,const Tok*t,int index){
  (void)index;if(t->len<2)return 0;uint8_t c=s[t->st];if(c<'A'||c>'Z')return 0;
  if(eq(s+t->st,t->len,"The")||eq(s+t->st,t->len,"This")||eq(s+t->st,t->len,"What")||eq(s+t->st,t->len,"Which")||eq(s+t->st,t->len,"How")||eq(s+t->st,t->len,"Did")||eq(s+t->st,t->len,"Is")||eq(s+t->st,t->len,"A")||eq(s+t->st,t->len,"An"))return 0;return 1;
}
static int entity_conflict(const Tok*gt,int ng,const uint8_t*gts,const Tok*ans,int na,const uint8_t*anss,const Tok*q,int nq,const uint8_t*qs){
  uint32_t known[32];int kc=0;for(int i=0;i<ng&&kc<32;i++)if(likely_entity(gts,&gt[i],i))known[kc++]=gt[i].h;for(int i=0;i<nq&&kc<32;i++)if(likely_entity(qs,&q[i],i))known[kc++]=q[i].h;if(kc==0)return 0;
  for(int i=0;i<na;i++)if(likely_entity(anss,&ans[i],i)){int ok=0;for(int j=0;j<kc;j++)if(known[j]==ans[i].h){ok=1;break;}if(!ok)return 1;}return 0;
}
static float char3(const uint8_t*a,int an,const uint8_t*b,int bn){
  if(an<3||bn<3)return 0.0f;if(an>2048)an=2048;if(bn>2048)bn=2048;int hit=0,total=0;
  for(int i=0;i<=an-3;i++){uint32_t h=hash(a+i,3);int seen=0;for(int j=0;j<=bn-3;j++)if(hash(b+j,3)==h){seen=1;break;}if(seen)hit++;total++;}
  int totalb=0;for(int j=0;j<=bn-3;j++){uint32_t h=hash(b+j,3);int seen=0;for(int i=0;i<=an-3;i++)if(hash(a+i,3)==h){seen=1;break;}if(seen)totalb++;}
  int den=total+totalb-hit;return den?((float)hit/(float)den):0.0f;
}
static float score_text(const uint8_t*q,int qn,const uint8_t*gt,int gn,const uint8_t*ans,int an){
  if(gn<=0||an<=0){return 0.0f;}if(!any_nonspace(ans,an))return 0.0f;
  Tok *g=gbuf,*a=abuf,*qt=qbuf;int ng=toks(gt,gn,g,MAX_TOKENS),na=toks(ans,an,a,MAX_TOKENS),nq=toks(q,qn,qt,MAX_TOKENS);if(ng<=0||na<=0)return 0.0f;
  if(exact_normalized_tokens(g,ng,a,na))return 1.0f;
  int common=0,stemc=0,semc=0,adj=0;
  for(int i=0;i<ng;i++){int found=0,sfound=0,sg=group(gt+g[i].st,g[i].len);for(int j=0;j<na;j++){if(g[i].h==a[j].h){found=1;sfound=1;break;}if(!sfound&&stem_match(gt+g[i].st,g[i].len,ans+a[j].st,a[j].len))sfound=1;}if(found)common++;if(sfound)stemc++;if(sg){for(int j=0;j<na;j++)if(group(ans+a[j].st,a[j].len)==sg){semc++;break;}}}
  for(int i=0;i+1<ng;i++)if(has(a,na,g[i].h)&&has(a,na,g[i+1].h))adj++;
  float prec=(float)common/(float)na,rec=(float)common/(float)ng,stem=(float)stemc/(float)ng,sem=(float)semc/(float)ng,phrase=(ng>1)?((float)adj/(float)(ng-1)):0.0f,cgram=char3(gt,gn,ans,an),len=(float)(gn<an?gn:an)/(float)(gn>an?gn:an);
  float score=.24f*prec+.25f*rec+.16f*stem+.15f*sem+.10f*phrase+.10f*cgram+.04f*len;
  if(ng<=3){int gg=group(gt+g[0].st,g[0].len);int same=0;for(int j=0;j<na;j++)if(group(ans+a[j].st,a[j].len)==gg){same=1;break;}if(gg&&same&&score<0.90f)score=0.90f;}
  int nqcommon=0;for(int i=0;i<nq;i++){if(group(q+qt[i].st,qt[i].len))continue;if(has(g,ng,qt[i].h))nqcommon++;}if(nq>0&&nqcommon>0)score+=0.03f;
  int nqnum=0;for(int i=0;i<qn;i++)if(digit(q[i])){nqnum=1;break;}if(nqnum){double tmp[1];int tp[1];if(parse_numbers(ans,an,tmp,tp,1)==0)score*=0.85f;}
  int nq4=numeric_quality(gt,gn,ans,an);if(nq4==1)score+=0.18f;else if(nq4<0)score*=0.32f;
  if(entity_conflict(g,ng,gt,a,na,ans,qt,nq,q))score*=0.10f;
  if(contradiction(gt,gn,g,ng,ans,an,a,na))score*=0.08f;
