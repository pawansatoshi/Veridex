...
sources: baseline seeds, independent seeds, and the official benchmark. It does
not modify official cases and it does not recursively mutate generated cases.
"""
...
def main() -> int:
    ...
    if args.rounds < 1 or args.rounds > 64:
        raise SystemExit("rounds must be between 1 and 64")

    base = []
    base.extend(load_cases(SEEDS, "seed"))
    base.extend(load_cases(INDEPENDENT, "independent-v2"))
    base.extend(load_cases(OFFICIAL, "official-benchmark"))
    ...
    for case in base:
        mutations = [
            ("number", mutate_number(case["good"])),
            ("direction", mutate_direction(case["good"])),
            ("entity", mutate_entity(case["good"])),
            ("relation", mutate_relation(case["good"])),
            ("incomplete", incomplete(case["good"])),
            ("late-contradiction", late_contradiction(case["good"])),
            ("distractor", distract(case["good"])),
            ("hedged", hedged(case["good"])),
        ...
        for round_id in range(args.rounds):
            for kind, bad in mutations:
                add_pair(output, seen, case, bad, f"generated-{kind}", round_id)

    source_counts = {
        source: sum(case["source"] == source for case in base)
        for source in ["seed", "independent-v2", "official-benchmark"]
    }
    payload = {
        "version": 4,
        "generator": "generate_shadow_corpus_v2.py",
        "rounds": args.rounds,
        "base_cases": len(base),
    ...
    print(json.dumps({"output_pairs": len(output), "base_cases": len(base), "source_counts": source_counts}, indent=2))
    return 0
