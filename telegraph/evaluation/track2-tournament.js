...
function checkedRange(mem, ptr, len, label) {
  if (!Number.isInteger(ptr) || ptr < 0 || !Number.isInteger(len) || len < 0 || ptr > mem.length || len > mem.length - ptr) {
    throw new Error(`${label}: invalid linear-memory range ${ptr}+${len} > ${mem.length}`);
  }
}
...
for (const c of data.cases) {
  ...
  for (const high of highs) {
    for (const low of lows) {
      ...
      sum += margin;
      margins.push(margin);
      values.push(sh.score, sl.score);
      worst = Math.min(worst, margin);
      best = Math.max(best, margin);
      if (margin > 0) wins++;
      else if (margin < 0) { losses++; inversions++; }
      else ties++;
      ...
      if (!(margin > 0)) {
        failures.push({
          ...
          high: high.label,
          highAnswer: high.text,
          highScore: sh.score,
          highComponents: sh.breakdown,
          low: low.label,
          lowAnswer: low.text,
          lowScore: sl.score,
          lowComponents: sl.breakdown,
          margin,
          likelyFailureMode: margin === 0 ? 'tie: insufficient separation' : 'inversion: high-quality answer scored below low-quality answer',
        });
      }
    }
  }
...
function freshScore() {
  ...
  try {
    ...
    return e2.rank_answer(pq, q.length, pg, gt.length, pa, a.length);
  } finally {
...
// Exercise UTF-8, embedded NUL, and a payload above 65,535 bytes.
...
const out = {
  ...
  bestMargin: Number.isFinite(best) ? best : 0,
  selfMatch: exact.score,
  scoreStddev,
  deterministicRepeat: d1 === d2,
  freshInstanceDeterministic: freshScore() === d1,
  invalidScoreCount: 0,
  runtimeMs: null,
