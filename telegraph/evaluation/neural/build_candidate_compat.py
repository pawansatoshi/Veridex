...
fn vr_question_predicate_conflict(q:&[u8],gt:&[u8],ans:&[u8])->bool{if !vr_question_is_binary(q){return false;}let gp=match vr_first_binary_polarity(gt){Some(v)=>v,None=>return false};let nq=vr_has_any(q,&[b\"denied\",b\"rejected\",b\"blocked\",b\"forbidden\",b\"unauthorized\",b\"unauthorised\",b\"unsafe\",b\"fraudulent\",b\"dangerous\"]);let pq=vr_has_any(q,&[b\"approved\",b\"authorized\",b\"authorised\",b\"allowed\",b\"permitted\",b\"safe\",b\"benign\",b\"legitimate\",b\"trusted\"]);let na=vr_has_any(ans,&[b\"denied\",b\"rejected\",b\"blocked\",b\"forbidden\",b\"unauthorized\",b\"unauthorised\",b\"unsafe\",b\"fraudulent\",b\"dangerous\"]);let pa=vr_has_any(ans,&[b\"approved\",b\"authorized\",b\"authorised\",b\"allowed\",b\"permitted\",b\"safe\",b\"benign\",b\"legitimate\",b\"trusted\"]);(nq&&((gp&&pa)||(!gp&&na)))||(pq&&((gp&&na)||(!gp&&pa)))}
fn vr_question_guard(q:&[u8],gt:&[u8],ans:&[u8])->f32{let mut g=1.0f32;if vr_question_requires_number(q)&&vr_first_number(ans).is_none(){g*=0.82;}if vr_question_is_binary(q){if let Some(p)=vr_first_binary_polarity(gt){match vr_first_binary_polarity(ans){Some(a)if a!=p=>g*=0.06,None=>g*=0.88,_=>{}}if vr_binary_fragment(ans){g*=0.35;}if vr_question_predicate_conflict(q,gt,ans){g*=0.06;}if (vr_has_word(gt,b\"compromised\")&&vr_has_word(ans,b\"secure\"))||(vr_has_word(gt,b\"secure\")&&vr_has_word(ans,b\"compromised\")){g*=0.06;}}}g}"""
