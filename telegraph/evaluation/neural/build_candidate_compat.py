...
The official baseline remains pinned and untouched. This adapter applies only
Veridex wrapper compatibility/hardening patches:
- Rust-2024-safe scratch-range sizing
- conservative penalty for binary-answer fragments such as ``No, it``
- ASCII case/punctuation normalization before semantic baseline scoring

The normalization is deliberately limited to ASCII surface form: letters are
lowercased and ordinary punctuation is converted to spaces, while numeric
syntax ($, %, comma, decimal point, minus, slash) is retained. This prevents
surface casing/punctuation from overwhelming semantic ranking without changing
Unicode content or the factual guards.
"""
...
NEW_BASE = """let q_base=vr_normalize_baseline_surface(q.as_bytes());let gt_base=vr_normalize_baseline_surface(gt.as_bytes());let a_base=vr_normalize_baseline_surface(a.as_bytes());
    let base=rank_answer_base(q_base.as_ptr() as i32,q_base.len() as i32,gt_base.as_ptr() as i32,gt_base.len() as i32,a_base.as_ptr() as i32,a_base.len() as i32);if !base.is_finite(){return(0.0,0.0,0.0,0.0);}"""

for needle,name in ((OLD_RANGE,"scratch-range expression"),(OLD_GUARD,"question-guard expression"),(OLD_SCORE,"score function prologue"),(OLD_BASE,"baseline scoring call")):
