...
const phrase = (s) => { const reps=[[/\bbullish\b/gi,'positive'],[/\bbearish\b/gi,'negative'],[/\bfraudulent\b/gi,'fraud'],[/\blegitimate\b/gi,'genuine'],[/\bmalicious\b/gi,'harmful'],[/\bdangerous\b/gi,'unsafe'],[/\bauthorized\b/gi,'approved'],[/\bunauthorized\b/gi,'not authorized'],[/\bincreased\b/gi,'rose'],[/\bdecreased\b/gi,'fell'],[/\breduced\b/gi,'dropped'],[/\bconfirmed\b/gi,'verified']]; let out=s; for(const [r,v] of reps) out=out.replace(r,v); return out; };
...
const directionSwap=(s)=>s.replace(/\b(increased|increase|rose|rise|rising|up|higher|gain|gained)\b/gi,'DECREASE_MARK').replace(/\b(decreased|decrease|fell|fall|falling|down|lower|declined|reduced|dropped|loss|lost)\b/gi,'INCREASE_MARK').replace(/DECREASE_MARK/g,'decreased').replace(/INCREASE_MARK/g,'increased');
const polarityFlip=(s)=>s.replace(/\b(safe|secure|benign|legitimate|genuine|trusted|authorized|approved|allowed|permitted|confirmed|bullish|positive)\b/gi,'POLARITY_POS').replace(/\b(unsafe|malicious|fraudulent|fraud|scam|dangerous|harmful|rejected|denied|blocked|forbidden|compromised|bearish|negative)\b/gi,'POLARITY_NEG').replace(/POLARITY_POS/g,'unsafe').replace(/POLARITY_NEG/g,'safe');

for (const c of src.cases) {
  const highs=c.answers.filter(a=>a.tier==='high'), lows=c.answers.filter(a=>a.tier==='low'); if(!highs.length||!lows.length) continue;
  const h=highs[0].text,l=lows[0].text;
...
fs.writeFileSync(output,JSON.stringify({intent:src.intent,cases},null,2));
